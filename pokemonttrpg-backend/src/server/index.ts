...
export interface Room {
  ...
  // For boss/team modes: real player IDs that need to submit team preview orders
  teamPreviewRealPlayerIds?: string[];
  // For boss/team modes: each real player's sub-team for team preview
  teamPreviewPlayerTeams?: Record<string, Player>;
  ...
  bossMode?: {
    ...
    pokemonOwnership: Map<string, string>;  // pokemonId → real player ID (for team preview with our format)
    pokemonOwnershipByIndex: string[];  // mergedTeam index → real player ID (for PS request filtering)
    detailsOwnerMap?: Map<string, string[]>;  // PS details string → owner player IDs (built on first prompt)
  ...
  teamBattleMode?: {
    sides: Array<{
      ...
      pokemonOwnershipByIndex: string[];
      detailsOwnerMap?: Map<string, string[]>;
    }>;
  };
}

type TeamPreviewParticipantInfo = {
  playerId: string;
  name: string;
  trainerSprite?: string;
  team: any[];
...
if (FUSION_SPRITES_DIR && fs.existsSync(FUSION_SPRITES_DIR)) {
  ...
  if (FUSION_GEN_REMOTE_BASE && fetchFn) {
    ...
    registerFusionGenRoutes(app, fusionGenService);
...
function loadBugReports(): any[] {
  try { return fs.existsSync(BUG_REPORTS_FILE) ? JSON.parse(fs.readFileSync(BUG_REPORTS_FILE, "utf-8")) : []; }
  catch { return []; }
}
function saveBugReports(reports: any[]) {
  fs.writeFileSync(BUG_REPORTS_FILE, JSON.stringify(reports, null, 2));
}

app.post("/api/bug-report", (req: Request, res: Response) => {
  try {
...
app.get("/api/bug-reports", (_req: Request, res: Response) => {
  const reports = loadBugReports();
  res.json(reports);
});

app.get("/api/bug-reports/:id", (req: Request, res: Response) => {
  const reports = loadBugReports();
...
function startTeamPreview(room: Room, players: Player[], rules?: any) {
  ...
  const buildTeamPreviewParticipants = (): Record<string, TeamPreviewParticipantInfo[]> => {
    const bySide: Record<string, TeamPreviewParticipantInfo[]> = { p1: [], p2: [] };
    ...
    if (room.bossMode) {
      ...
      if (soloPlayer) {
        bySide[soloSide] = [{
          ...
          trainerSprite: (soloPlayer as any).trainerSprite || (soloPlayer as any).avatar,
      ...
      if (mergedPlayer) {
        bySide[room.bossMode.mergedSide] = room.bossMode.playerSlots.map((slotInfo) => {
          ...
          for (const [pokId, ownerId] of room.bossMode!.pokemonOwnership) {
            if (ownerId === slotInfo.playerId) ownedPokemonIds.add(pokId);
          }
          const ownedTeam = (mergedPlayer.team || []).filter((p: any) => ownedPokemonIds.has(p.id));
    ...
    for (let i = 0; i < players.length; i++) {
      ...
      bySide[sideId] = [{
        ...
        trainerSprite: (player as any).trainerSprite || (player as any).avatar,
        team: player.team || [],
        previewActiveCount: activeCount,
      }];
    }
  ...
  const emitTeamPreviewToPlayer = (playerId: string, side: string, team: any[], maxTeamSize: number, previewActiveCount: number) => {
    ...
    sock.emit("promptAction", {
      ...
      prompt: {
        ...
        side: {
          ...
          pokemon: team.map((p: any, idx: number) => ({
  ...
  const playerTeams: Record<string, Player> = {};
  ...
  if (room.bossMode) {
    ...
    for (let i = 0; i < players.length; i++) {
      const player = players[i];
      if (player.id === room.bossMode.mergedPlayerId) continue; // Handle merged side below
      const side = `p${i + 1}`;
  ...
  // Team battle mode: split both sides' team previews to individual allies
    ...
    for (let i = 0; i < players.length; i++) {
      ...
      for (const slotInfo of sideConfig.playerSlots) {
        ...
        const ownedTeam = player.team.filter((p: any) => ownedPokemonIds.has(p.id));
        const maxTeamSize = rules?.maxTeamSize || Math.min(6, ownedTeam.length);
      ...
      if (player.id === room.bossMode!.mergedPlayerId) {
        ...
        for (let slot = 0; slot < maxLen; slot++) {
          for (const subTeam of reorderedSubTeams) {
            if (slot < subTeam.length) mergedTeam.push(subTeam[slot]);
          }
        }
        const clone = JSON.parse(JSON.stringify(player)) as Player;
...
function launchChallenge(sourceRoom: Room, challenge: Challenge) {
  ...
  if (isFFA) {
    ...
    const pokemonOwnershipByIndex: string[] = [];
...
io.on("connection", (socket: Socket) => {
  ...
  socket.on("requestBattleState", (data: { roomId: string }) => {
    const room = data?.roomId ? rooms.get(data.roomId) : undefined;
