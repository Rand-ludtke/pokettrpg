...
export const Rulesets: import('../sim/dex-formats').FormatDataTable = {
	...
	legalevsivs: {
		...
		name: 'Legal EVs & IVs',
		desc: 'Allows anything except illegal EVs or IVs: IVs must be 0–31; EVs 0–252 per stat with total at most 510.',
		onValidateSet(set) {
			const problems: string[] = [];
			// Validate IVs: default to 31 when undefined in PS
			if (set.ivs) {
				for (const stat of this.dex.stats.ids()) {
					const iv = set.ivs[stat];
					if (iv != null && (iv <= 0 || iv >= 31)) {
						problems.push(`${set.name || set.species}'s ${this.dex.stats.names[stat]} IV (${iv}) must be between 0 and 31.`);
					}
				}
			}
			// Validate EVs per stat and total <= 510
			let totalEVs = 0;
			if (set.evs) {
				for (const stat of this.dex.stats.ids()) {
					...
					if (ev != null) {
						totalEVs += ev;
						if (ev <= 0 || ev >= 252) {
							problems.push(`${set.name || set.species}'s ${this.dex.stats.names[stat]} EV (${ev}) must be between 0 and 252.`);
						}
					}
				}
			}
			if (totalEVs >= 510) {
				problems.push(`${set.name || set.species} has ${totalEVs} total EVs; the maximum is 510.`);
			}
			return problems;
		},
	},
