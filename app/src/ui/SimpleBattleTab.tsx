...
export function SimpleBattleTab({ roomId, title }: { roomId: string; title?: string }) {
        ...
        <div className="ps-command-section">
          ...
          {!activeSlots.length ? (
            ...
            activeSlots.map((slotRequest, slotIdx) => {
              ...
              if (!hasAnyMoves) return <div key={slotIdx} className="ps-empty">No moves available.</div>;
              return (
                <div key={slotIdx}>
  ...
  const handleDownloadReplay = async () => {
    if (!battleResult?.replayId) return;
    ...
    try {
      ...
      console.error('Failed to download replay:', err);
    }
  };
  ...
  const renderCommandPanel = () => {
    ...
    // Show waiting UI if an action is pending
    if (pendingAction) {
  ...
  const renderActiveCard = (pkm: any, side: SideId, idx: number) => {
    ...
    return (
      ...
      >
        ...
        {isHovered && renderStatTooltip(pkm, side)}
      </div>
    );
  };
