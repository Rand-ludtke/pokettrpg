...
export const TRAINER_TRAITS: TrainerTrait[] = [
  ...
  { name: 'Chef', desc: 'During a short rest, if you cook and spend extra supplies, the whole party restores full SP instead of half. Each trainer also regains 1d8 HP, and each Pokémon in the current team regains 1/16 of its max HP, rounded up. This can only trigger once per short rest.', reqText: '10 INT', minStats: { intelligence: 10 } },
  ...
  { name: 'Jr. Professor', desc: 'When you use Intelligence for a Wild Bond, you gain advantage. If you meet the Respect DC but miss the Straight-catch DC, increase your final Bond Bonus by +1.', reqText: 'D12 INT (12 INT)', minStats: { intelligence: 12 } },
  { name: 'Nurse', desc: 'Medicine has double effective uses when you apply it: split one item into two normal doses, or spend one dose for double healing. You gain advantage on crafting medicine and on Field Treatment checks. Also gives advantage on Wild Bond attempts when dealing with an injured Pokémon.', reqText: 'D12 INT (12 INT)', minStats: { intelligence: 12 } },
  ...
  { name: 'Dual Specialty', desc: 'Choose a second Type Specialty. Outsider penalty applies only outside both. Matching either specialty gives your normal bonus. If a Pokémon matches both (dual-typed), treat it as a double bonus (double catch modifier and check bonuses). In TTRPG battles, dual-matching Pokémon gain 1.25x HP and battle bonding is 0.75x easier.', reqText: '15 INT + Expanding Horizons', minStats: { intelligence: 15 }, requiredTraits: ['Expanding Horizons'] },
