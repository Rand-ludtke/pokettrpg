  ...
  return (
    <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:12}}>
      <div style={{display:'flex', flexDirection:'column', gap:6}}>
        ...
        <textarea rows={8} placeholder='{ "name": "Alcremie", ... }' value={speciesText} onChange={e=>setSpeciesText(e.target.value)} />
        <label>learnset.json (optional)</label>
        <textarea rows={6} placeholder='{ "dazzlinggleam": ["9L1"], ... }' value={learnsetText} onChange={e=>setLearnsetText(e.target.value)} />
      </div>
      <div style={{display:'flex', flexDirection:'column', gap:6}}>
        ...
        <p className="dim" style={{ marginBottom: 0 }}>Tip: Provide Showdown-style species and learnset JSONs.</p>
      </div>
    </div>
  );
}
