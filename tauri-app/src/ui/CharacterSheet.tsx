...
export function CharacterSheet() {
		...
		(async()=>{
			try {
				...
				try {
					...
					Object.keys(custom || {}).forEach(k => { map[toID(k)] = { ...(custom as any)[k], id: toID(k) }; });
				} catch {}
				setItemData(map);
			} catch {}
		})();
	...
	useEffect(()=>{
		...
		const loadFromBattleAvatarNumbers = () => {
			const nums = (window as any)?.BattleAvatarNumbers;
			if (!nums || typeof nums !== 'object') return [] as string[];
			return dedupeAndSort(Object.values(nums).map(String));
		};
		const loadFromManifest = async () => {
			try {
				const res = await fetch(withPublicBase('vendor/showdown/sprites/trainers.json'));
				if (!res.ok) return [] as string[];
				const json = await res.json();
				if (Array.isArray(json)) return dedupeAndSort(json.map(String));
				if (Array.isArray((json as any)?.list)) return dedupeAndSort((json as any).list.map(String));
				return [] as string[];
			} catch {
				return [] as string[];
			}
		};
		const loadFromBattleDataFile = async () => {
			try {
				const res = await fetch(withPublicBase('vendor/showdown/js/battledata.js'));
				if (!res.ok) return [] as string[];
				const text = await res.text();
				const match = text.match(/BattleAvatarNumbers\s*=\s*\{([\s\S]*?)\};/);
		...
		async function load() {
			if (api?.listTrainers) {
				try {
					const r = await api.listTrainers();
					if (cancelled) return;
					if (r?.ok && Array.isArray(r.list)) {
						setTrainerOptions(dedupeAndSort(r.list));
						return;
					}
				} catch {}
			}
			...
			if (fromManifest.length) {
				if (!cancelled) setTrainerOptions(fromManifest);
				return;
			}
			const fromGlobals = loadFromBattleAvatarNumbers();
			if (fromGlobals.length) {
				if (!cancelled) setTrainerOptions(fromGlobals);
				return;
			}
			const fromFile = await loadFromBattleDataFile();
			if (!cancelled) setTrainerOptions(fromFile);
		}
		load();
	...
	function renderItemIcon(entry: any, size=20) {
		...
		if (typeof entry.sprite === 'string') {
			return <img src={entry.sprite} alt={entry.name || 'item'} style={{ width: size, height: size, imageRendering: 'pixelated' }} />;
		}
		if (typeof entry.spritenum !== 'number') return null;
						...
						<div style={{ display:'grid', gridTemplateColumns:'140px 1fr', gap:12, alignItems:'center', marginTop:8 }}>
							<div style={{ width:140, height:140, border:'1px solid var(--accent)', borderRadius:8, background:'transparent', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden' }}>
								{trainerImage ? (
									...
									<img src={withPublicBase(`vendor/showdown/sprites/trainers/${trainerSprite}.png`)} alt="Sprite" style={{ imageRendering:'pixelated', width:'80%', height:'80%', objectFit:'contain', background:'transparent' }} />
								)}
							</div>
									...
									{trainerOptions
										...
										.map((name)=> (
											<button key={name} title={name} className={trainerSprite===name? 'active':''} onClick={()=>{ applyTrainerSprite(name); setShowSpritePicker(false); }} style={{width:64, height:64, padding:0, border: trainerSprite===name? '2px solid var(--acc)': '1px solid #444', borderRadius:4, background:'transparent', display:'flex', alignItems:'center', justifyContent:'center'}}>
												<img src={withPublicBase(`vendor/showdown/sprites/trainers/${name}.png`)} alt={name} style={{ width:48, height:48, imageRendering:'pixelated', background:'transparent' }} />
											</button>
										))}
								</div>
							</>
