...
function normalizeApiBase(raw: string | null | undefined): string {
  ...
  try {
    ...
    if (
      hostname === LEGACY_NIPIO_HOST ||
      hostname === LEGACY_NIPIO_HOST_2 ||
      hostname === LEGACY_DUCKDNS_HOST
    ) {
      url.protocol = 'https:';
...
const DEFAULT_API_BASE = 'https://pokettrpg-app.pokemondnd.xyz';
const LEGACY_DUCKDNS_HOST = 'pokettrpg.duckdns.org';
const LEGACY_NIPIO_HOST = '47-218-210-137.nip.io';
const LEGACY_NIPIO_HOST_2 = '108-92-243-110.nip.io';
const CURRENT_HOST = 'pokettrpg-app.pokemondnd.xyz';
...
function isGithubPagesRuntime(): boolean {
  if (typeof window === 'undefined' || isTauriApp()) return false;
...
export class PoketTRPGClient {
  ...
  /** Tracks in-flight sendAction calls awaiting server confirmation */
  private pendingSendAction: { roomId: string; playerId: string; action: any; retries: number; timer: ReturnType<typeof setTimeout> } | null = null;
  /** Last measured round-trip latency in ms (from Socket.io engine ping) */
  ...
  /** Get measured round-trip latency in ms (from Socket.io pings), or null if unknown */
  ...
  connect(username: string) {
    ...
    // Tauri desktop apps aren't subject to mixed-content restrictions
    if (!isTauriApp() && typeof window !== 'undefined' && window.location?.protocol === 'https:' && endpoint.startsWith('http://')) {
    ...
    try {
      const engine = (socket as any).io?.engine ?? (socket as any).engine;
      if (engine) {
    ...
    socket.on('connect', () => {
      ...
      this.identifyIfNeeded();
      // On reconnect, rejoin all active battle rooms so the server restores our player slot
      // and re-sends current battle state + prompts
      this.rejoinActiveBattleRooms();
    });

    socket.on('disconnect', () => {
      this.updateStatus('idle');
      // Cancel any in-flight action delivery tracking — will retry after reconnect
    ...
    socket.on('battleUpdate', (update: any) => {
      // Server processed the turn — our action was delivered
      this.cancelPendingSendAction();
  ...
  /** Get or create a persistent user ID for reconnection support */
  private getPersistentUserId(): string {
    const key = 'pokettrpg-user-id';
  ...
  private identifyIfNeeded() {
    ...
    const payload: any = { username: this.username, userId: this.getPersistentUserId() };
    if (trainerSprite) {
  ...
  sendAction(roomId: string, action: BattleAction, playerId?: string) {
    ...
    // Cancel any previous in-flight retry for this player
    this.cancelPendingSendAction();
    this.socket?.emit('sendAction', { roomId, playerId: pid, action: enriched });
    // Track delivery — expect server to respond with promptAction(wait) or battleUpdate
    // within a timeout. If neither arrives, retry the action.
    this.pendingSendAction = {
      roomId,
      playerId: pid,
      action: enriched,
      retries: 0,
      timer: setTimeout(() => this.retrySendAction(), 4000),
    };
  }

  /** Retry a sendAction if no server acknowledgment arrived */
  private retrySendAction() {
    const pending = this.pendingSendAction;
    if (!pending) return;
    if (pending.retries >= 2) {
      // Give up after 2 retries — server or connection likely broken
      console.warn('[Client] sendAction delivery failed after retries:', pending.roomId, pending.playerId);
      this.pendingSendAction = null;
      return;
    }
    pending.retries++;
    console.log(`[Client] Retrying sendAction (attempt ${pending.retries + 1}):`, pending.roomId, pending.playerId);
    this.socket?.emit('sendAction', { roomId: pending.roomId, playerId: pending.playerId, action: pending.action });
    pending.timer = setTimeout(() => this.retrySendAction(), 4000 * (pending.retries + 1));
  }

  /** Cancel in-flight sendAction tracking (called on ack, battleUpdate, or disconnect) */
  private cancelPendingSendAction() {
    if (this.pendingSendAction) {
      clearTimeout(this.pendingSendAction.timer);
      this.pendingSendAction = null;
    }
  }
  ...
  private rejoinActiveBattleRooms() {
    // Rejoin lobby
    this.joinRoom(LOBBY_ROOM_ID, 'player');
    // Rejoin any battle rooms we have state for — the server's joinRoom handler
    // detects disconnectedPlayers and restores our slot, re-sends battleStarted + prompts
    for (const roomId of this.battleStates.keys()) {
      if (roomId === LOBBY_ROOM_ID) continue;
      this.joinRoom(roomId, 'player');
      // Also explicitly request state recovery in case joinRoom doesn't trigger it
      this.socket?.emit('requestBattleState', { roomId });
    }
    // Also rejoin rooms we auto-joined previously
    for (const roomId of this.autoJoinedBattleRooms) {
      if (roomId === LOBBY_ROOM_ID) continue;
      if (this.battleStates.has(roomId)) continue; // already handled above
      this.joinRoom(roomId, 'player');
    }
  }
